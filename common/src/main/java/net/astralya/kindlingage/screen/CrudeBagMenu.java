...
public final class CrudeBagMenu extends AbstractContainerMenu {
  ...
  public CrudeBagMenu(int syncId, Inventory playerInventory) {
    this(
        syncId, playerInventory, playerInventory.player, resolveClientHand(playerInventory.player));
  }
  ...
  public CrudeBagMenu(int syncId, Inventory playerInventory, Player player, InteractionHand hand) {
    ...
    this.container = new CrudeBagInventory(bagStack, player.level().registryAccess());
    ...
    for (int col = 0; col < COLUMNS; col++) {
      this.addSlot(new BagSlot(container, col, containerX + col * 18, containerY));
    }
  ...
  public ItemStack quickMoveStack(Player player, int index) {
    ...
    if (index < containerEnd) {
      ...
      if (!this.moveItemStackTo(stackInSlot, 0, containerEnd, false)) {
        return ItemStack.EMPTY;
      }
    }
